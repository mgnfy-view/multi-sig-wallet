...
def test_only_owners_can_execute_txns(owners, not_owner, wallet, issue_eth_txn):
...
@pytest.mark.txn_execution
def test_passing_an_invalid_txn_index_reverts(owners, wallet, issue_eth_txn):
    issue_eth_txn(owners[0])
...
@pytest.mark.txn_execution
def test_eth_txn_execution_transfers_eth(
...
@pytest.mark.txn_execution
def test_txn_emits_txn_executed_event_on_success(owners, wallet, issue_eth_txn):
    issue_eth_txn(owners[0])
    wallet.approveTxn(0, 0, sender=owners[0])
    wallet.approveTxn(0, 0, sender=owners[1])
    owners[0].transfer(wallet.address, "2 ether")
...
@pytest.mark.txn_execution
def test_txns_can_only_be_executed_once(owners, wallet, issue_eth_txn):
    issue_eth_txn(owners[0])
    wallet.approveTxn(0, 0, sender=owners[0])
    wallet.approveTxn(0, 0, sender=owners[1])
    owners[0].transfer(wallet.address, "2 ether")
